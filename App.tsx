...
const generateMockRides = (): Ride[] => {
  ...
  for (let i = 0; i < 15; i++) {
     ...
     rides.push({
        id: `r${idCounter++}`,
        driver: { ...MOCK_USER_TEMPLATE, firstName: driver.name.split(' ')[0], lastName: driver.name.split(' ')[1], avatar: driver.avatar, rating: driver.rating, totalRides: driver.rides, isVerified: driver.verified, role: 'driver' },
        origin: origin, destination: dest, stops: [], departureTime: new Date(date), arrivalTime: new Date(date.getTime() + 3600000 * 3), price: 45, currency: 'CAD', seatsAvailable: Math.floor(Math.random() * 3) + 1, luggage: { small: 1, medium: 1, large: 0 },
        features: { instantBook: true, wifi: true, music: true, pets: false, smoking: false, winterTires: true }, distanceKm: 250, description: `Heading to ${dest.split(',')[0]} for work. Smooth driver.`
     });
  }
    ...
    <div className="h-full flex flex-col items-center justify-center p-6 bg-slate-50 relative overflow-hidden">
      ...
      <div className="w-full max-w-sm relative z-10">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 font-medium">Join the journey.</p>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl p-8 rounded-[2.5rem] shadow-2xl shadow-indigo-100 border border-white">
          <div className="flex mb-8 bg-slate-100 p-1.5 rounded-2xl">
             <button onClick={() => setIsSignUp(true)} className={`flex-1 py-3 text-sm font-bold rounded-xl transition-all ${isSignUp ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>Sign Up</button>
             <button onClick={() => setIsSignUp(false)} className={`flex-1 py-3 text-sm font-bold rounded-xl transition-all ${!isSignUp ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>Log In</button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
             ...
             <Button type="submit" className="mt-6 w-full">{isSignUp ? 'Create Account' : 'Welcome Back'}</Button>
          </form>
        </div>
      </div>
    </div>
  ...
  if (needsOnboarding) {
     ...
     return (
        <div className="h-full bg-slate-50 pb-32 overflow-y-auto px-6 pt-12">
           <Header title="Driver Setup" subtitle="Let's get you on the road" />
      ...
      <div className="px-6 relative -top-12">
        <div className="bg-white rounded-[2.5rem] p-8 shadow-2xl border border-slate-50">
           ...
           <div className="mt-8 flex items-center gap-4 bg-slate-50 p-4 rounded-2xl">
              ...
              <button className="p-3 bg-white rounded-xl shadow-sm"><Phone size={20} className="text-slate-400"/></button>
           </div>
           
           <Button className="mt-8 w-full shadow-2xl shadow-indigo-500/30">Book This Ride</Button>
        </div>
      </div>
    </div>
  );
...
const App: React.FC = () => {
  ...
  const renderView = () => {
    switch(currentView) {
      case 'home': case 'search': return <HomeView setView={setView} setDetailRide={setSelectedRide} lang={lang} setLang={setLang} user={user} allRides={allRides} />;
